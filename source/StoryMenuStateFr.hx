...
class StoryMenuStateFr extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		scoreText.text = "SEMAINE SCORE:" + lerpScore;
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			...
			PlayState.SONG = Song.loadFromJson(PlayStateFr.storyPlaylist[0].toLowerCase() + diffic, PlayStateFr.storyPlaylist[0].toLowerCase());
			PlayState.storyWeek = curWeek;
			PlayState.campaignScore = 0;
			PlayState.campaignMisses = 0;
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				LoadingState.loadAndSwitchState(new PlayStateFr(), true);
				FreeplayState.destroyFreeplayVocals();
                                FreeplayStateFr.destroyFreeplayVocals();
			});
		} else {
			FlxG.sound.play(Paths.sound('cancelMenu'));
		}
